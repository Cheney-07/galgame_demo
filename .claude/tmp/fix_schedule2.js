...
// Fix 1: Remove emoji and capture slot_name by value (replace both _show_save_slots and _show_load_slots)

// Replace _show_save_slots
const oldSave = [
...
const newSave = [
	...
	'',
	'\t\t\tvar idx := i',
	'\t\t\tbtn.pressed.connect(_do_schedule_save.bind(slot_name, idx))',
	'\t\t\tgrid.add_child(btn)',
].join('\n');

if (content.includes(oldSave)) {
	content = content.replace(oldSave, newSave);
	console.log('OK: _show_save_slots updated');
} else {
	console.log('ERROR: _show_save_slots pattern not found');
}

// Replace _show_load_slots
const oldLoad = [
...
const newLoad = [
	...
	'\t\t\t\tbtn.add_theme_stylebox_override("normal", _make_flat_style(Color(0.15, 0.15, 0.25, 0.85)))',
	'\t\t\t\tvar idx := i',
	'\t\t\t\tbtn.pressed.connect(_do_schedule_load.bind(slot_name, idx))',
	'\t\telse:',
	'\t\t\tbtn.text = "空栏位 " + str(i + 1)',
	'\t\t\tbtn.disabled = true',
	'\t\t\tbtn.add_theme_color_override("font_color", Color(0.5, 0.5, 0.5))',
...
if (content.includes(oldLoad)) {
	...
	console.log('OK: _show_load_slots updated');
} else {
	console.log('ERROR: _show_load_slots pattern not found');
}
